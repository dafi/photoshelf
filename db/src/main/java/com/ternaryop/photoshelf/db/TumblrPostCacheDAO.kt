...
class TumblrPostCacheDAO internal constructor(
    dbHelper: SQLiteOpenHelper
) : AbsDAO<TumblrPostCache>(dbHelper), BaseColumns {
    ...
    override fun getContentValues(pojo: TumblrPostCache): ContentValues {
        ...
        try {
            ...
            v.put(POST, toBlob(checkNotNull(pojo.post)))
        } catch (e: IOException) {
            throw DBException(e)
        }
    ...
    fun deleteItem(item: TumblrPost): Int = dbHelper.writableDatabase
        .delete(TABLE_NAME, BaseColumns._ID + "=? and " + BLOG_NAME + "=?", arrayOf(item.reblogKey, item.blogName))

    fun insertItem(post: TumblrPost, cacheType: Int): Long {
        return insertWithOnConflict(TumblrPostCache(post.reblogKey, post, cacheType))
    }
    ...
    fun delete(postIds: Collection<String>, cacheType: Int, blogName: String) {
        ...
        val whereClause = BaseColumns._ID +
            " in (" + inClauseParameters(postIds.size) + ") and " + BLOG_NAME + "=? and " + CACHE_TYPE + "=?"
